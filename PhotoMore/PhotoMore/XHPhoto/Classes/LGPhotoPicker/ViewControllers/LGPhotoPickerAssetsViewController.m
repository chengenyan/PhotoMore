...
@interface LGPhotoPickerAssetsViewController () <LGPhotoPickerCollectionViewDelegate,UICollectionViewDataSource,LGPhotoPickerBrowserViewControllerDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate>
...
- (UIButton *)sendBtn{
    if (!_sendBtn) {
        ...
        NSString *title = [NSString stringWithFormat:@"完成(%d%ld)",0,self.maxCount];
...
- (void) setupToorBar{
    ...
    NSString *heightVfl = @"V:[toorBar(49)]-0-|";
...
- (void)setMaxCount:(NSInteger)maxCount {
    ...
    if (self.sendBtn) {
        NSString *title = [NSString stringWithFormat:@"完成(%d/%ld)",0,maxCount];
...
- (void)updateToolbar
{
    ...
    self.sendBtn.enabled = (count > 0);
    self.previewBtn.enabled = (count > 0);
    NSString *title = [NSString stringWithFormat:@"完成(%ld/%ld)",(long)count,self.maxCount];
    [self.sendBtn setTitle:title forState:UIControlStateNormal];
}
