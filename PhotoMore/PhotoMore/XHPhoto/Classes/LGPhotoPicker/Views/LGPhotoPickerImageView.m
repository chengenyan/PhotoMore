...
- (UIImageView *)tickImageView {
    if (!_tickImageView) {
        ...
        tickImageView.image = [UIImage imageNamed:@"wphoto_select_no"];
...
- (void)setMaskViewFlag:(BOOL)maskViewFlag {
    ...
    if (!maskViewFlag) {
        // hidden
        [self.tickImageView setImage:[UIImage imageNamed:@"wphoto_select_no"]];
    }else{
        [self.tickImageView setImage:[UIImage imageNamed:@"wphoto_select_yes"]];
    }
